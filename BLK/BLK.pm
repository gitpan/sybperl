...
#    my $cols 	= $self->{_blk_global}->{FIELDS};   #### IGNORED!
    ...
    my %reorder = %{$self->{_blk_global}->{REORDER}} if(defined($self->{_blk_global}->{REORDER}));
	...
	for ($i = 0; $i < @tabinfo; ++$i) {
	    if ($tabinfo[$i]->[1] =~ /datetim/ && !defined($cols{$i}->{CALLBACK})) {
		$cols{$i}->{CALLBACK} = $date_fmt{$self->{_blk_global}->{DATE}};
	    }
	}
    }
...
sub _gettabinfo {
    ...
    croak "Must specify the Sybase table as database.user.table" if (!defined($tab));
