...
for results. If it finds a DBPROCESS that is ready it returns it, along with 
the reason why it's ready. If dbpoll() times out, or if an interupt occurs
$dbproc will be undefined, and $reason will be either DBTIMEOUT or DBINTERUPT.
If $millisecs is 0 then dbpoll() returns immediately. If $millisecs is -1 then
it will not return until either results are pending or a system interupt has
occured. Please see the Sybase documentation for further details.
...
B<Registered procedure execution:>
...
$value is the input value if $action is B<CS_GET>, and the output
...
Michael Peppler E<lt>F<mpeppler@mbay.net>E<gt>
