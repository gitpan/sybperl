<--$Id: sybperl.pod,v 1.42 2001/10/29 19:24:50 mpeppler Exp $-->
...
=head2 EXPERIMENTAL Bulk-Copy Routines (BLK API)

The following routines allow you to use a small subset of the blk_*()
...
CS_BULK_LOGIN property to the new() or ct_connect() call:
   ...
   my $dbh = new Sybase::CTlib $user, $pwd, $server, $appname, {CON_PROPS => {CS_BULK_LOGIN => CS_TRUE }};
...
These routines are B<experimental>, and the API may change in the future.
...
It's on the end of a 56K line, so it may be slow (depending on what I happen
...
The sybperl mailing list E<lt>F<sybperl-l@list.cren.net>E<gt> is the
