...
time. The L<Sybase::BCP> module adds two attributes to the normal
...
L<Special handling of DATETIME, MONEY & NUMERIC/DECIMAL values>
below for details.
...
Please see also L<Common Sybase::DBlib and Sybase::CTlib routines> below.
...
=item $dbh = new Sybase::DBlib [$user [, $server [, $appname [, {additional attributes}]]]]
...
See also the L<Sybase::BCP> module.
...
precision. See the L<Sybase::DBlib::Money> discussion below.
...
See also L<Sybase::BCP> for a symplified bulk copy API.
...
Additional B<attributes> can be set by passing a hash reference.
...
B<TYPE>, B<MAXLENGTH>, B<SCALE>, B<PRECISION>, B<STATUS>.
...
=item $ret = ct_config($action, $property, $value)

Calls ct_config() to change some basic parameter, like the
<B>interfaces</B> file location.

$action can be <B>CS_SET</B> or <B>CS_GET</B>.
...
$value is the input value if $action is <B>CS_GET</B>, and the output
value if $action is <B>CS_GET</B>.

$ret is the return status of the ct_config() call.

Example:

	$ret = ct_config(CS_SET, CS_IFILE, "/home/mpeppler/foo");
	print "$ret\n";

	$ret = ct_config(CS_GET, CS_IFILE, $out);
...
Michael Peppler E<lt>F<mpeppler@itf.ch>E<gt>
...
The sybperl mailing list E<lt>F<sybperl-l@trln.lib.unc.edu>E<gt> is the
